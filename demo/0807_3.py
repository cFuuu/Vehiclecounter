...
class Vehicle:
    def __init__(self, position):
        self.positions = deque(maxlen=1)  # 保存最近5個位置
...
def vehicle_count(video_path):
    ...
    detection_zones = [
        ...
        #{"coords": [350, 375, 550, 385], "color": (255, 0, 0), "count": 0},    # 藍色區間
        #{"coords": [550, 420, 735, 440], "color": (0, 255, 102), "count": 0},  # 綠色區間
        #{"coords": [740, 460, 980, 490], "color": (0, 255, 255), "count": 0},  # 黃色區間
        #{"coords": [980, 500, 1250, 550], "color": (0, 165, 255), "count": 0}, # 橙色區間
        # 可以繼續添加更多區間
    ]
    ...
    while True:
        ...
        for contour in contours:
            if cv2.contourArea(contour) > 3000:  # 調整閾值
                # 獲取邊界矩形
...
zone_counts, total_count = vehicle_count(video_path)
